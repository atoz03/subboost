...
function buildLocalSubscriptionConfig(
  ...
  const migratedExistingConfig = migrateFilteredProxyGroupsConfig(existingConfig);
  const config = normalizeSubscriptionConfigForPersistence(
    ...
    {
      existingConfig: migratedExistingConfig,
  ...
  // 旧版筛选分组迁移期间，旧客户端可能只提交它认识的配置字段。
  // 保留迁移生成的分组，避免一次普通保存把已迁移的节点关系静默删除。
  if (
    isRecord(body.config) &&
    !("customProxyGroups" in body.config) &&
    Array.isArray(existingConfig.filteredProxyGroups) &&
    Array.isArray(migratedExistingConfig.customProxyGroups)
  ) {
    const migratedGroups = migratedExistingConfig.customProxyGroups.filter(
      (group) => isRecord(group) &&
        typeof group.id === "string" &&
        group.id.startsWith("migrated-filtered-")
    );
    if (migratedGroups.length > 0) config.customProxyGroups = migratedGroups;
  }
